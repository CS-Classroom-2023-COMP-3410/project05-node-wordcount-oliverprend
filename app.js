...
function readFileContent() {
    ...
    const content = fs.readFileSync("declaration.txt", "utf-8")
    return content;
}
...
function getWordCounts(content) {
    ...
    const wordCount = {};
    // needed to add lowercase to this one
    const words = content.toLowerCase().split(/\W+/).filter(Boolean); // Splitting by non-word characters.

    // if word is a key then add one to counter
    // if not a key already then set it equal to 1
